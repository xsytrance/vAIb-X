...
```bash
node server/api.mjs      # port 3014 — REST + agent discovery + music
npm run dev              # port 3013 — Vite/React UI
```

Both bind to `0.0.0.0` — accessible at `http://<your-host-ip>:3013`.
...
- **Port conflict**: `relay.mjs` also tries port 3014. Don't run relay and api simultaneously without changing `RELAY_PORT`.
