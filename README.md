...
vAIb/
├── server/
│   ├── api.mjs          # REST backend — state, agents, music endpoints (port 666)
...
# Terminal 1 — API server (port 666)
node server/api.mjs

# Terminal 2 — UI dev server (port 667)
npm run dev
```

UI → `http://localhost:667`
...
|---|---|
| `VAIB_PORT` | API server port (default: 666) |
